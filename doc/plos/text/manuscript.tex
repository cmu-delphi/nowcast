...
information which is not available at runtime. In any case, MASE is a useful
measure for comparing the performance of a given nowcast across different
locations.

Given the above discussion of the Naive Oracle, we define the naive nowcast for
any given location on any given week as the final value of ground truth in the
same location one week earlier. Using the same notation as above, and
considering the $m$ weeks for which ground truth is available on the prior
week, MASE is defined as:
