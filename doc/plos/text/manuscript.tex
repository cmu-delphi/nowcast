...
MAE of a naive nowcast in the same location. We define the naive nowcast for
any given location on any given week as the reported ground truth in the same
location 52 weeks earlier. Using the same notation as above, and considering
the $m$ weeks for which ground truth is available for at least 52 weeks prior,
MASE is defined as:

\begin{align*}
  \text{MASE} &= \text{MAE} \left( \frac{1}{m} \sum_{w=1}^m{\left|x_w - x_{w - 52}\right|} \right) ^ {-1}
\end{align*}
