...
In a given location over a given period of time, we assess the standalone
accuracy of a nowcast by two measures: mean absolute error (MAE) root mean
squared error (RMSE). With ground truth vector $\vec{x}$, nowcast vector
$\vec{y}$, and elements indexed by week $w$ of $n$, these measures are defined
as:
...
the normal range of wILI varies between locations. To facilitate comparison
between locations, we also report mean absolute scaled error (MASE)
\cite{hyndman2006another}. MASE is the MAE of a given nowcast normalized by the
MAE of a one-step naive nowcast computed in-sample in the same location.
...
within a narrow set of locations and seasons). As a result, we introduce the
...
the Naive Oracle is unfairly advantaged. As such, MASE is reasonably subject to
being greater than one, indicating that the nowcast under consideration
performed \textit{worse} than the ``naive'' nowcast. This should be
unsurprising, if not expected, given that the Naive Oracle incorporates
information which is not available at runtime. In any case, while MASE is not
particularly helpful as an absolute measure of accuracy, MASE is useful for
comparing relative accuracy between different locations.

We define the naive nowcast for any given location on any given week as the
final value of ground truth in the same location one week earlier. Using the
same notation as above, and considering the $m$ weeks for which ground truth is
available on the prior week, MASE is defined as:
